<x-app-layout>
    ...
    <div class="py-6 space-y-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <!-- Stock Allocation Requests & Approval Status -->
            <div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
                <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <h3 class="text-base font-bold text-slate-900 dark:text-white">Stock Allocation Requests & Status</h3>
                        <p class="text-xs text-slate-500">Track status of product requests submitted directly or by your Sales Representative</p>
                    </div>
                    <a href="{{ route('stock-requests.index') }}" class="inline-flex items-center text-xs font-semibold text-emerald-600 dark:text-emerald-400 hover:underline">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60 text-xs">
                            @forelse($stockRequests ?? [] as $req)
                                <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors">
                                    ...
                                    <td class="py-4 px-6">
                                        @if($req->isPending())
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-amber-50 text-amber-700 dark:bg-amber-950 dark:text-amber-300">
                                                ⏳ Pending Review
                                            </span>
                                        @elseif($req->isApproved())
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">
                                                ✓ Approved & Assigned
                                            </span>
                                        @else
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-rose-50 text-rose-700 dark:bg-rose-950 dark:text-rose-300">
                                                ✗ Rejected
                                            </span>
                                        @endif
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="py-8 text-center text-slate-400 dark:text-slate-500">
                                        No stock allocation requests found. Click <a href="{{ route('stock-requests.create') }}" class="text-emerald-600 underline">Request Stock</a> to request new product inventory.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
